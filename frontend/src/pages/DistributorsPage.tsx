...
export default function DistributorsPage() {
  ...
  const draftKey = useMemo(
    () => makeDraftKey({ kind: 'distributorDraft', tenantSlug: effectiveSlug || getTenantSlug(), userId: user?.id }),
  ...
  function isDraftEmpty(d: any) {
    if (!d) return true
    if (String(d.supplierName || '').trim()) return false
    if (String(d.dlNumber || '').trim()) return false
    if (String(d.address || '').trim()) return false
    return true
  }
  ...
  useEffect(() => {
    ...
    const currentEmpty = isDraftEmpty({ supplierName, dlNumber, address })
    if (!currentEmpty) return
    setSupplierName(String(draft.supplierName || ''))
    setDlNumber(String(draft.dlNumber || ''))
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      const payload = { v: 1, updatedAt: Date.now(), supplierName, dlNumber, address }
      if (isDraftEmpty(payload)) clearDraft(draftKey)
      else saveDraft(draftKey, payload)
    }, 800)
    return () => window.clearTimeout(t)
  }, [draftKey, token, user?.id, canManage, supplierName, dlNumber, address])
  ...
  useEffect(() => {
    setPage(1)
  }, [searchText, filterActive, sortKey, sortDir])
